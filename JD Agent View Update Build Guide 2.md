...
**Rule 4: Capture changes, don't rewrite the JD.** During an update, the agent never shows a rewritten section or a full updated JD. It turns what the manager says into a short, specific change overview per section (Add / Remove / Change / Reword). The manager confirms that overview, and it's what gets stored and sent to HR. HR stays the author of the final JD language, and managers aren't asked to proofread AI-written text they might approve without reading.
...
| 8 | Summarize Requested Change | Prompt tool | New | Topics only |
...
"Update the purpose of Branch Manager to include SBA lending"
  ...
  → change loop (describe → confirm a short change overview → repeat for other sections)
     (the agent never shows a rewritten section or a full updated JD)
...
| Current_Purpose / Requested_Purpose | Multi-line plain text ×2 | Current section text vs. the confirmed change overview |
| Current_KSA / Requested_KSA | Multi-line plain text ×2 | |
| Current_Duties / Requested_Duties | Multi-line plain text ×2 | |
| Current_Education / Requested_Education | Multi-line plain text ×2 | |
| Current_Certs / Requested_Certs | Multi-line plain text ×2 | |
| ManagerNotes | Multi-line plain text | The manager's original wording for each change, including any text they pasted |
...
**Inputs (all text):** `JobTitle`, `CurrentClassifications`, `CurrentJD`, `RequestedChanges`, `Criteria`, `ArchitectureReference`
...
REQUESTED CHANGES (only these sections are changing; each shows the current text and the manager's requested changes):
{RequestedChanges}

INSTRUCTIONS
1. Evaluate all six categories against the criteria. Judge the whole job as it would be once the requested changes are applied, not only the individual change items.
2. Flag a category when the requested changes add, remove, or materially change duties or requirements the criteria use for that classification. This includes changes that move the job toward qualifying AND away from qualifying.
...
| RequestedPurpose, RequestedKSA, RequestedDuties, RequestedEducation, RequestedCerts | Text | No (empty = section unchanged). Each holds that section's confirmed change overview |
...
5. **Compose `RequestedChanges`**: one block per changed section. Pattern for one section (repeat for all five inside a single `concat`):
   ```
   if(empty(triggerBody()?['RequestedPurpose']), '',
     concat('SECTION: Purpose', decodeUriComponent('%0A'),
            'CURRENT: ', coalesce(body('Get_item')?['Purpose'],''), decodeUriComponent('%0A'),
            'REQUESTED CHANGES: ', triggerBody()?['RequestedPurpose'], decodeUriComponent('%0A%0A')))
   ```
...
10. **Condition:** `RequestId` is empty or 0?
    ...
    - Both write: Title, JobKey, requester fields, ChangedSections, `Current_X` (from Get item) and `Requested_X` for changed sections only (blank for unchanged), ManagerNotes, ImpactFlagged, FlagCategories, FlagDetails_User, FlagDetails_HR, TitleFitConcern, AssessmentSummary_HR, ConversationId.
...
| Summarize Requested Change (prompt) | Only when referenced by topics |
...
- Never write out a revised section or a full updated JD, during or after an update request. Updates are captured as a short overview of requested changes for HR. If a user asks to see the final version, explain that HR finalizes the wording after review, and offer to show the current JD.
...
### 5.1 Summarize Requested Change
**Inputs:** `JobTitle`, `SectionName`, `CurrentText`, `ChangeRequest`, `ExistingChanges`. **Output:** text.

This prompt does **not** rewrite the section. It turns the manager's words into a short, specific list of changes HR can act on.
```
You help people managers request changes to job descriptions. Turn the manager's request into a short, specific list of changes for HR. Do NOT rewrite the section.
...
Changes already requested for this section:
{ExistingChanges}
...
Rules:
1. Output 1 to 6 bullets. Each starts with "- " followed by one of: Add:, Remove:, Change:, Reword:.
2. Be specific enough that HR can act without asking. Name the exact duty, skill, requirement, or license. For Change items, state what it changes from and to (e.g., "- Change: Bachelor's degree from required to preferred").
3. Each bullet is 25 words or fewer. Never output the full rewritten section or long quotes from it.
4. If the manager pasted their own version of the section, compare it to the current text and list only the substantive differences. Summarize pure wording or ordering edits as a single "- Reword:" bullet.
5. If changes were already requested for this section, return the combined list: keep earlier items, add the new ones, merge duplicates, and when the new request conflicts with an earlier item, keep the new one.
6. Keep the manager's intent. Do not add changes they didn't ask for.
7. If the request is too vague to act on, output exactly: CLARIFY: followed by one short question.
8. Output only the bullet list.
```
...
**Topic variables:** `JobKey`, `JobTitleSel`, `JD` (record), `ChgPurpose`, `ChgKSA`, `ChgDuties`, `ChgEducation`, `ChgCerts` (the confirmed change overview for each section), `Notes`, `SelectedField`, `PrefillText`, `ChangeOverview`, `RequestId` (start 0), `FirstPass` (start true).
...
2. Message: *"Let's update **{JobTitleSel}**. Tell me what should change in the Purpose, Principal Duties, KSAs, Education, or Certifications, and I'll put together a short summary for HR. HR finalizes the wording, and nothing changes until they approve it."*
3. **Set** `Topic.PrefillText = Topic.RequestedChange` so a change described in the opening message carries into the first card.
...
2. Otherwise, **Set** `Topic.ChangedCount`:
   ```
   CountIf(Table({v:Topic.ChgPurpose},{v:Topic.ChgKSA},{v:Topic.ChgDuties},{v:Topic.ChgEducation},{v:Topic.ChgCerts}), !IsBlank(v))
   ```
3. **Ask with adaptive card:**
   ```
   {
     ...
     body: [
       { type: "TextBlock", weight: "Bolder", wrap: true,
         text: "Which section of " & Topic.JobTitleSel & " needs changes?" },
       { type: "Input.ChoiceSet", id: "field", style: "compact", placeholder: "Choose a section",
         choices: [
           { title: "Purpose" & If(IsBlank(Topic.ChgPurpose), "", "  ✓ changes added"), value: "Purpose" },
           { title: "Principal Duties & Responsibilities" & If(IsBlank(Topic.ChgDuties), "", "  ✓ changes added"), value: "Duties" },
           { title: "Knowledge, Skills & Abilities" & If(IsBlank(Topic.ChgKSA), "", "  ✓ changes added"), value: "KSA" },
           { title: "Education" & If(IsBlank(Topic.ChgEducation), "", "  ✓ changes added"), value: "Education" },
           { title: "Certifications & Licenses" & If(IsBlank(Topic.ChgCerts), "", "  ✓ changes added"), value: "Certs" } ] },
       { type: "TextBlock", isSubtle: true, wrap: true,
         text: If(Topic.ChangedCount = 0, "No changes yet.", Text(Topic.ChangedCount) & " section(s) with changes so far.") }
     ],
     actions: [
       { type: "Action.Submit", title: "Change a section", data: { action: "edit" } },
...
4. **Condition on action:**
   ...
   - `done` + `ChangedCount = 0` → "You haven't added any changes yet." → back to ChooseSection. Otherwise go to **Step G**.
   - `cancel` → go to **Step I** (cancel path).
...
1. **Set** helper variables:
   - `Topic.FieldLabel`:
     ...
     ```
   - `Topic.PendingChanges` (changes already confirmed for this section):
     ```
     Switch(Topic.SelectedField, "Purpose",Topic.ChgPurpose, "Duties",Topic.ChgDuties, "KSA",Topic.ChgKSA, "Education",Topic.ChgEducation, "Certs",Topic.ChgCerts)
     ```
   - `Topic.CurrentText`:
     ```
     ...
     ```
2. **Ask with adaptive card.** The current text is shown for reference only; the manager describes what should change.
   ```
   {
     type: "AdaptiveCard", version: "1.5",
     body: [
       { type: "TextBlock", weight: "Bolder", wrap: true, text: Topic.FieldLabel & " (current text)" },
       { type: "TextBlock", wrap: true, isSubtle: true,
         text: If(IsBlank(Topic.CurrentText), "This section is currently empty.", Topic.CurrentText) },
       { type: "Container", spacing: "Medium", isVisible: !IsBlank(Topic.PendingChanges), items: [
           { type: "TextBlock", weight: "Bolder", wrap: true, text: "Changes you've already added here" },
           { type: "TextBlock", wrap: true, text: Coalesce(Topic.PendingChanges, " ") } ] },
       { type: "Input.Text", id: "instruction", isMultiline: true, value: Topic.PrefillText,
         placeholder: "What should change? e.g., 'add oversight of SBA loan referrals' or 'remove the cash drawer duty'. You can also paste your own version." },
       { type: "ActionSet", isVisible: !IsBlank(Topic.PendingChanges), actions: [
           { type: "Action.Submit", title: "Remove all changes to this section", data: { action: "clear" } } ] }
     ],
     actions: [
   ...
   ```
3. **Set** `Topic.FirstPass = false` and `Topic.PrefillText = ""`.
4. **Condition on action:**
   - `back` → ChooseSection.
   - `clear` → five Set variable nodes, e.g. `Topic.ChgPurpose = If(Topic.SelectedField = "Purpose", Blank(), Topic.ChgPurpose)` (same pattern for the other four) → message "Removed your changes to **{FieldLabel}**." → ChooseSection.
   - `continue` with blank instruction → "Tell me what should change first." → Describe.

#### Step F: Summarize and confirm
1. Call **Summarize Requested Change**:
   - `JobTitle = Topic.JobTitleSel`, `SectionName = Topic.FieldLabel`, `CurrentText = Topic.CurrentText`
   - `ChangeRequest = Topic.instruction`
   - `ExistingChanges = If(IsBlank(Topic.PendingChanges), "None", Topic.PendingChanges)`
   - Output → `Topic.ChangeOverview`
2. **Condition:** `StartsWith(Topic.ChangeOverview, "CLARIFY:")` → message `Mid(Topic.ChangeOverview, 10)` → `Set Topic.PrefillText = Topic.instruction` (so they add to what they wrote instead of retyping) → Describe.
3. **(Label: Confirm)** Ask with adaptive card:
   ```
   {
     type: "AdaptiveCard", version: "1.5",
     body: [
       { type: "TextBlock", weight: "Bolder", wrap: true, text: "Here's what I'll send HR for " & Topic.FieldLabel },
       { type: "Input.Text", id: "overview", isMultiline: true, value: Topic.ChangeOverview },
       { type: "TextBlock", isSubtle: true, wrap: true,
         text: "Adjust the wording if anything's off. HR writes the final JD language." }
     ],
     actions: [
       { type: "Action.Submit", title: "Looks good", data: { action: "accept" } },
       { type: "Action.Submit", title: "Change something", data: { action: "revise" } },
       { type: "Action.Submit", title: "Discard", data: { action: "discard" } }
     ]
   }
   ```
   The overview already includes earlier changes to this section (the prompt merges them), so what's in the box is the complete list for the section.
4. **Condition on action:**
   - **`accept`** (if `Topic.overview` is blank, treat as discard): five **Set variable** nodes in a row, no branching needed:
     ```
     Topic.ChgPurpose   = If(Topic.SelectedField = "Purpose",   Topic.overview, Topic.ChgPurpose)
     Topic.ChgDuties    = If(Topic.SelectedField = "Duties",    Topic.overview, Topic.ChgDuties)
     Topic.ChgKSA       = If(Topic.SelectedField = "KSA",       Topic.overview, Topic.ChgKSA)
     Topic.ChgEducation = If(Topic.SelectedField = "Education", Topic.overview, Topic.ChgEducation)
     Topic.ChgCerts     = If(Topic.SelectedField = "Certs",     Topic.overview, Topic.ChgCerts)
     ```
     Then `Topic.Notes = Topic.Notes & Topic.FieldLabel & ": " & Topic.instruction & Char(10) & Char(10)` (the manager's original wording, kept for HR), message "Added to your request." → ChooseSection.
   - **`revise`**: `Set Topic.PrefillText = Topic.instruction` → Describe.
   - **`discard`**: message "Discarded. Any changes you added to this section earlier are still in your request." → ChooseSection.

   *Why five variables instead of a table:* there are exactly five editable sections, each holds one change list, re-confirming a section naturally replaces its list (the prompt has already merged old and new items), and it sidesteps Power Fx's lack of a simple "append to table" in Copilot Studio. To add Work Experience later: one more variable, one more card choice, one more flow input.
...
2. Call **Assess JD Change Impact**:
   ...
   - `ChgPurpose`, `ChgKSA`, `ChgDuties`, `ChgEducation`, `ChgCerts` → the five `Requested*` inputs
...
4. **Set** `Topic.ChangeSummary`, the overview used on every remaining card and the final message:
   ```
   Concatenate(
     If(!IsBlank(Topic.ChgPurpose), "**Purpose**" & Char(10) & Topic.ChgPurpose & Char(10) & Char(10), ""),
     If(!IsBlank(Topic.ChgDuties), "**Principal Duties & Responsibilities**" & Char(10) & Topic.ChgDuties & Char(10) & Char(10), ""),
     If(!IsBlank(Topic.ChgKSA), "**Knowledge, Skills & Abilities**" & Char(10) & Topic.ChgKSA & Char(10) & Char(10), ""),
     If(!IsBlank(Topic.ChgEducation), "**Education**" & Char(10) & Topic.ChgEducation & Char(10) & Char(10), ""),
     If(!IsBlank(Topic.ChgCerts), "**Certifications & Licenses**" & Char(10) & Topic.ChgCerts & Char(10) & Char(10), ""))
   ```
...
```
{
  ...
  body: [
    ...
    { type: "ActionSet", actions: [
        { type: "Action.ToggleVisibility", title: "Show my requested changes",
          targetElements: [ { elementId: "summaryBlock" } ] } ] },
    { type: "TextBlock", id: "summaryBlock", isVisible: false, wrap: true, text: Topic.ChangeSummary }
  ],
  actions: [
...
```
{
  type: "AdaptiveCard", version: "1.5",
  body: [
    { type: "TextBlock", weight: "Bolder", size: "Medium", wrap: true, text: "Ready to send to HR" },
    { type: "TextBlock", wrap: true, text: Topic.ChangeSummary },
    { type: "TextBlock", wrap: true, isSubtle: true,
      text: "No classification concerns were detected. HR will still review before anything changes." }
  ],
  actions: [
...
```
`submit` → Finalize with `SubmissionType = "NoFlags"`. `edit` → ChooseSection. `cancel` → Step I (cancel path).

#### Step I: Close out
- **Submitted:** message (an overview only; never the updated JD):
  > Your request **{RequestNumber}** for **{JobTitleSel}** has been sent to HR. Here's what you asked for:
  >
  > {ChangeSummary}
  >
  > HR will finalize the wording and follow up if they need anything else.
...
- Clear `Global.LastJobKey` / `Global.LastJobTitle` → **End all topics**. Ending all topics also stops the orchestrator from adding its own follow-up reply after the topic, which is where an unwanted "here's your updated JD" would otherwise come from.
...
| "Add SBA referrals to the Branch Manager purpose" | Job, section, and change pre-filled; short change overview shown for confirmation, no rewritten text |
| Add two separate changes to Purpose | Second request merges into one Purpose change list |
| Vague instruction ("make it better") | CLARIFY question, back to describe step |
| Pastes their own full version of Duties | Overview lists only the substantive differences |
| "Show me the updated JD" during or after an update | No rewritten JD; explains HR finalizes wording; offers the current JD |
...
- Everything it needs is on the row: the current text and the confirmed change overview per section, the manager's original wording, user and HR flag detail, title-fit concern, justification, submission type, requester.
- If you want the report to include a drafted revised section (e.g., highlighted in the JD template), generate it inside the report flow for HR. That keeps rewritten JD text out of the manager's conversation.
...
| Keep a value unless selected | `If(Topic.SelectedField = "Purpose", Topic.overview, Topic.ChgPurpose)` |
| Strip a prefix | `Mid(Topic.ChangeOverview, 10)` |
